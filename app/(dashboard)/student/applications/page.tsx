...
import { formatDateTime, trimSeconds } from '@/lib/time';
					...
					{applications.map((app) => {
						const selectedTimeSummaries =
							app.choices?.map((choice) => {
								...
								return `${days[win.day_of_week]} ${trimSeconds(win.start_time)} - ${trimSeconds(win.end_time)}${
									win.instructor_name
										? ` · ${win.instructor_name}`
										: ''
								}`;
							...
							<div
								...
								<div>
									...
									{requestedTimeSummaries.length > 0 && (
										<p className='text-xs text-slate-600'>
											신청한 시간:{' '}
											{requestedTimeSummaries.map(
												(summary, idx) => (
													<span
														key={`${app.id}-request-${idx}`}>
														{idx > 0 && ', '}
														{summary}
													</span>
												),
