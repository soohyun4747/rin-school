...
type SortableField = 'name' | 'birthdate' | 'country' | 'created_at' | 'guardian_name';

export default async function AdminStudentsPage({
        searchParams,
}: {
        searchParams: Promise<{ q?: string; page?: string; sort?: SortableField; dir?: string }>;
}) {
        const { profile } = await requireSession();
        requireRole(profile.role, ['admin']);
        const supabase = await getSupabaseServerClient();

        const { q = '', page = '1', sort = 'created_at', dir = 'desc' } =
        ...
        const allowedSortFields: SortableField[] = ['name', 'birthdate', 'country', 'created_at', 'guardian_name'];
        const sortBy: SortableField = allowedSortFields.includes((sortParam ?? 'created_at') as SortableField)
	...
	const totalPages = Math.max(1, Math.ceil(totalCount / pageSize));
        ...
        const buildSortLink = (field: SortableField) => {
                ...
                params.set('dir', sortBy === field && sortDirection === 'asc' ? 'desc' : 'asc');
                return `?${params.toString()}`;
        };
							...
							<table className='min-w-full divide-y divide-slate-200 text-sm'>
                                                                <thead className='bg-slate-50'>
                                                                        <tr>
                                                                                ...
                                                                                <th className='px-4 py-2 text-left'>
                                                                                        {renderSortLabel('생년월일', 'birthdate')}
                                                                                </th>
                                                                                <th className='px-4 py-2 text-left text-xs font-semibold text-slate-600'>카카오톡 ID</th>
                                                                                <th className='px-4 py-2 text-left'>
                                                                                        {renderSortLabel('거주지', 'country')}
                                                                                </th>
                                                                                <th className='px-4 py-2 text-left'>
                                                                                        {renderSortLabel('학부모 이름', 'guardian_name')}
                                                                                </th>
                                                                                <th className='px-4 py-2 text-left'>
