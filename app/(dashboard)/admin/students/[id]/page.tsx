...
export default async function AdminStudentDetailPage({ params }: { params: { id: string } }) {
        ...
        const applications: ApplicationRow[] = (applicationResult.data as ApplicationRow[] | null) ?? [];
                ...
                <div className='space-y-4'>
                        <div className='flex items-center justify-between'>
                                <div>
                                        ...
                                        <p className='text-sm text-slate-600'>학생 정보와 신청 내역을 확인하세요.</p>
                                </div>
                                <Link
                                        href='/admin/students'
                                ...
                                <CardContent className='space-y-3 text-sm text-slate-700'>
                                        <div className='grid gap-4 sm:grid-cols-2'>
                                                <div>
                                                        ...
                                                        <p className='font-semibold text-slate-900'>{student.username || '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>이메일</p>
                                                        <p className='font-semibold text-slate-900 break-all'>{student.email}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>연락처</p>
                                                        <p className='font-semibold text-slate-900'>{student.phone || '연락처 없음'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>생년월일</p>
                                                        <p className='font-semibold text-slate-900'>{student.birthdate ?? '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>카카오톡 ID</p>
                                                        <p className='font-semibold text-slate-900'>{student.kakao_id ?? '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>거주지</p>
                                                        <p className='font-semibold text-slate-900'>{student.country ?? '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>학부모 이름</p>
                                                        <p className='font-semibold text-slate-900'>{student.guardian_name ?? '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>학부모 이메일</p>
                                                        <p className='font-semibold text-slate-900'>{consent?.guardian_email ?? '미입력'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>보호자 동의 상태</p>
                                                        <p className='font-semibold text-slate-900'>{consent?.guardian_status ?? '미확인'}</p>
                                                </div>
                                                <div>
                                                        <p className='text-xs text-slate-500'>가입일</p>
                                                        <p className='font-semibold text-slate-900'>{formatDateTime(new Date(student.created_at))}</p>
                                                </div>
                                        </div>
                                </CardContent>
                        </Card>
                                                        ...
                                                        <div
                                                                ...
                                                                <div className='flex items-center justify-between gap-2'>
                                                                        ...
                                                                        {app.course?.id ? (
                                                                                ...
                                                                                <span className='text-xs text-slate-400'>연결된 수업 없음</span>
                                                                        )}
                                                                </div>
