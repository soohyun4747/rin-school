...
export default async function AdminCourseDetailPage({
  ...
  const { data: courseData } = await supabase
    .from("courses")
    .select("id, title, subject, grade_range, description, duration_minutes, capacity, image_url, weeks")
    .eq("id", id)
    .single();
  ...
  const [{ data: windows }, { data: applications }, { data: matches }] = await Promise.all([
    ...
    supabase
      .from("applications")
      .select("id, student_id, status, created_at, application_time_choices(window_id)")
      .eq("course_id", course.id)
      .order("created_at", { ascending: false }),
    supabase
      .from("matches")
      .select("id, instructor_id, instructor_name, slot_start_at, slot_end_at, status, match_students(student_id)")
      .eq("course_id", course.id)
  ...
  const { data: profiles } = profileIds.size
    ? await supabase.from("profiles").select("id, name, phone").in("id", Array.from(profileIds))
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          {windowsRows.length === 0 && <p className="text-slate-600">등록된 시간이 없습니다.</p>}
          {windowsRows.map((w) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {applicants.length === 0 ? (
                    <p className="text-xs text-slate-600">아직 신청한 학생이 없습니다.</p>
                  ) : (
                    <ul className="space-y-2">
                      {applicants.map((app) => (
                        <li key={`${w.id}-${app.id}`} className="flex items-center justify-between rounded-md border border-slate-100 px-3 py-2">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">
                              {profileMap.get(app.student_id)?.name ?? app.student_id}
                            </p>
                            <p className="text-xs text-slate-600">
                              {profileMap.get(app.student_id)?.phone ?? "연락처 없음"} · {app.status}
                            </p>
                          </div>
                          <span className="text-[11px] text-slate-500">{formatDateTime(new Date(app.created_at))}</span>
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-semibold text-slate-600">선택 시간</th>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {applicationRows.map((app) => {
                    const selections = app.application_time_choices ?? [];
                    return (
                      <tr key={app.id} className="hover:bg-slate-50">
                        <td className="px-4 py-2 font-semibold text-slate-900">
                          {profileMap.get(app.student_id)?.name ?? "학생"}
                        </td>
                        <td className="px-4 py-2 text-slate-700">
                          {profileMap.get(app.student_id)?.phone ?? "연락처 없음"}
                        </td>
                        <td className="px-4 py-2 text-slate-700">
                          {selections.length === 0
                            ? "선택 없음"
                            : selections
                                .map((choice) => {
                                  const w = windowsRows.find((win) => win.id === choice.window_id);
                                  return w ? windowLabel(w) : "삭제된 시간";
                                })
                                .join(", ")}
                        </td>
