// lib/matching.ts
import {
	getSupabaseServerClient /*, getSupabaseAdminClient */,
	...
	try {
		const [
			...
			supabase
				.from('applications')
				.select(
					'id, student_id, created_at, application_time_choices(window_id)'
				)
				.eq('course_id', courseId)
				.eq('status', 'pending')
			...
			supabase
				.from('matches')
				.select(
					'id, slot_start_at, slot_end_at, instructor_id, instructor_name, match_students(student_id)'
				)
				.eq('course_id', courseId)
		...
		if (courseError || !course)
			throw courseError || new Error('수업 정보를 불러오지 못했습니다.');
		...
		const occurrences = generateWindowOccurrences(normalizedSlots, {
			from: new Date(from),
			to: new Date(to),
		});
		...
		const occurrenceStates = occurrences
			.map((occ) => {
				const occStartTime = `${occ.start
					.getHours()
					.toString()
					.padStart(2, '0')}:${occ.start
					.getMinutes()
					.toString()
					.padStart(2, '0')}`;
				const windowInfo = normalizedSlots.find(
					(w) =>
						w.windowId === occ.windowId &&
						w.start_time === occStartTime &&
						w.day_of_week === occ.start.getDay()
				);
				if (!windowInfo) return null;
				const key = `${
					windowInfo.instructor_id ?? 'none'
				}-${occ.start.toISOString()}`;
				const match = existingMatches?.find(
					(m) =>
						m.slot_start_at === occ.start.toISOString() &&
						(m.instructor_id ?? 'none') ===
							(windowInfo.instructor_id ?? 'none')
				);
				const remaining =
					course.capacity - (match?.match_students?.length ?? 0);
				return {
					window: windowInfo,
					start: occ.start,
					end: occ.end,
		...
		for (const app of applications ?? []) {
			const choices = (app.application_time_choices ?? []).map(
				(c) => c.window_id
			);
			let matched = false;
			...
			for (const windowId of choices) {
				const candidates = occurrenceStates
					.filter(
						(occ) => occ.window.id === windowId && occ.remaining > 0
					)
				...
				if (!matchId) {
					const { data: match, error: matchInsertError } =
						await supabase
							.from('matches')
							.insert({
								course_id: courseId,
