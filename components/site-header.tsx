      ...
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-4 px-4 py-4">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex flex-wrap items-center gap-3 text-sm font-medium text-slate-700">
            <Link href="/" className="rounded-md px-2 py-1 hover:text-[var(--primary)]">
              홈
            </Link>
            <Link href="/classes" className="rounded-md px-2 py-1 hover:text-[var(--primary)]">
              수업
            </Link>
            {isLoggedIn ? (
              <Link href="/dashboard" className="rounded-md px-2 py-1 hover:text-[var(--primary)]">
                대시보드
              </Link>
            ) : (
              <Link href="/auth/login" className="rounded-md px-2 py-1 hover:text-[var(--primary)]">
                로그인
              </Link>
            )}
            {roleMenu.map((item) => (
