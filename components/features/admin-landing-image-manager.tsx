...
export function AdminLandingImageManager({
  ...
  const initialPathOrder = useMemo(() => images.map((item) => item.path).join('|'), [images]);
  ...
  const handleDrop = (targetIndex: number) => {
    ...
    setOrderedImages((prev) => reorderArray(prev, draggingIndex, targetIndex));
    setDraggingIndex(null);
  };
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-[var(--primary-border)] bg-white p-4 shadow-sm">
      ...
      {orderedImages.length > 0 ? (
        <>
          <p className="text-xs text-slate-500">카드를 드래그해서 노출 순서를 변경한 후, 순서 저장 버튼을 눌러주세요.</p>
          <form action={reorderAction} className="space-y-2">
            <input type="hidden" name="variant" value={variant} />
            <input
              type="hidden"
              name="orderedPaths"
              value={JSON.stringify(orderedImages.map((image) => image.path))}
            />
            <Button type="submit" size="sm" disabled={!hasOrderChanged}>
              순서 저장
            </Button>
          </form>
