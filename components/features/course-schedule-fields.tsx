...
export function CourseScheduleFields({
	...
	/**
	 * ✅ FIX: confirm/alert 같은 side-effect를 setState 업데이터 함수 안에서 호출하지 않기
	 * (React 18 dev StrictMode에서 업데이터 함수가 2번 호출될 수 있어서 confirm이 두 번 뜸)
	 */
	const addAllWeekWindows = () => {
		...
		for (
			let slotStart = 0;
			slotStart + durationMinutes <= maxDayMinute;
		...
		const nextWindows: TimeWindowField[] = weekDaysFromMonday.flatMap(
			(dayOfWeek) =>
		...
		<div className='md:col-span-2 space-y-4 rounded-lg border border-slate-200 p-4'>
			<div className='flex flex-col gap-1'>
				...
				<p className='text-xs text-slate-600'>
					최소 1개 이상의 시간 범위를 등록해주세요. 담당 강사를 지정할 수
					있습니다.
				</p>
			</div>
